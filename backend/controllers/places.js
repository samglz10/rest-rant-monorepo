...
router.post('/', async (req, res) => {
    if (req.currentUser?.role !== 'admin') {
        return res.status(403).json('You are not allowed to add a Place')
    }
    if (!req.body.pic) {
...
router.post('/:placeId/comments', async (req, res) => {
    ...
    if (!req.currentUser) {
        return res.status(404).json({message: `You must be logged in to leave a comment homie...`})
    }
...
router.delete('/:placeId/comments/:commentId', async (req, res) => {
    if (req.currentUser?.role !== 'admin') {
        return res.status(403).json('You are not allowed to delete a Place')
    }
    let placeId = Number(req.params.placeId)
    ...
    if (isNaN(placeId)) {
        ...
        if (!comment) {
            ...
            res.status(403).json({
                message: req.currentUser?.userId 
            });
        } else {
